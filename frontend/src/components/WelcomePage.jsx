...
const WelcomePage = ({ onGetStarted }) => {
  return (
    <div className="min-h-screen w-screen relative overflow-hidden bg-black font-sans">
      ...
      <div className="pointer-events-none absolute inset-0 opacity-[0.06]" style={{ backgroundImage: 'radial-gradient(rgba(255,255,255,0.28) 1px, transparent 1px)', backgroundSize: '22px 22px' }} />
      ...
      {/* Centered Hero (keep original structure) */}
        ...
        <div className="text-center max-w-5xl w-full">
          ...
          <div className="mt-6 mx-auto max-w-3xl rounded-2xl bg-white/5 border border-white/10 p-5 backdrop-blur-md shadow-xl animate-fadeUp">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
              {['Fast detection','Privacy-first','Real rewards','Student friendly'].map((t,i)=> (
                <span key={i} className="px-3 py-1 rounded-full text-xs md:text-sm bg-white/10 text-gray-100 border border-white/10">
                  {t}
                </span>
              ))}
            </div>
          ...
          <div className="mt-8 flex items-center justify-center gap-4">
            ...
            >
              ...
              <span className="relative flex items-center gap-2">Get Started <span>🚀</span></span>
            </button>
            <a href="#how" className="px-8 py-4 rounded-full bg-white/10 text-gray-100 border border-white/10 font-semibold hover:bg-white/20 transition-all duration-300">
              How it works
            </a>
          </div>
          ...
          <div className="mt-8 grid grid-cols-3 gap-3 max-w-xl mx-auto text-center">
            {[{n:'100K+',l:'Detections'},{n:'3K+',l:'Rewards Claimed'},{n:'50K+',l:'Items Recycled'}].map((s, i)=> (
              <div key={i} className="rounded-xl bg-white/5 border border-white/10 p-4">
                <div className="text-xl md:text-2xl font-extrabold text-gray-100 font-display">{s.n}</div>
      ...
      <section id="how" className="relative z-10 px-6 md:px-12 mt-16 md:mt-24">
        <div className="grid md:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {[
            ...
            { icon: '🤖', title: 'AI Detects', desc: 'YOLO instantly identifies recyclable / hazardous items.' },
      ...
      <footer className="relative z-10 px-6 md:px-12 mt-16 mb-12">
        <div className="max-w-4xl mx-auto text-center">
          <h3 className="text-gray-100 text-2xl md:text-3xl font-extrabold font-display">Ready to earn while you recycle?</h3>
          <p className="text-gray-300/90 mt-2">Join the squad, start detecting, and grab rewards.</p>
          <button onClick={onGetStarted} className="mt-6 px-8 py-3 rounded-full bg-eco-green text-white font-bold hover:brightness-110 transition">
            I’m In 🚀
          </button>
        </div>
      </footer>
